...
const assignStockSchema = Joi.object({
    stock_id: Joi.string().min(3).required(),
    team_id: Joi.string().min(3).required(),
    quantity: Joi.number(),
}).unknown();
...
export const updateStock= async (req,res)=>{
    ...
    try {
        ...
        const updateStock=await Stocks.findOneAndReplace({ _id: id }, stock, { new: true });


        if(!updateStock)
            return res.status(400).json({ message: "Some thing went wrong..." });
        
        return res.status(200).json({ message: "Stocks updated sucessfully...",updateStock });
...
export const assignStock= async (req,res)=>{
    const stock=req.body;
    try {
        const { error, value } = assignStockSchema.validate(req.body);
        if (error) {
            // Return a 400 Bad Request response if validation fails
            return res.status(400).json({ message: error.details[0].message });
        }
        ...
        if(existingStock.quantity<stock.quantity){
            return res.status(400).json({ message: "Requested Quantity is not available for selected stock..."});
        }


        const existingAssignedStock=await assignedStockModel.findOne({
        ...
        if(existingAssignedStock){
            ...
            existingAssignedStock.quantity+=parseInt(stock.quantity);

            await existingAssignedStock.save();
            await existingStock.save();
            return res.status(200).json({ message: "Stock Assigned..."});
        }

        const assignStock=new assignedStockModel(stock);
        const saveAssignStock=await assignStock.save();
        if(!saveAssignStock)
            return res.status(400).json({ message: "Some thing went wrong..." });
        
        return res.status(200).json({ message: "Stock Assigned sucessfully..." });
